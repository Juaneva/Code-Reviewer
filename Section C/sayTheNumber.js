let single = {
  ...
  function numToWord(number) {
      ...
      while (number.startsWith("0") & number.length !== 1) {
        ...
        if (number.length === 3) { // If it is 3 digits long, find hundreth
          tempString += single[number.slice(0, 1)] + " hundred and ";
          number = number.slice(1,);
        } 
        if (number.length === 2 && teens[number]) { // If it is a teen add
          tempString += teens[number];
        } else if (number.length === 2) { // 2 digit long, but not a teen
          if (number.slice(-1) === "0") { // If it ends with 0, add
            tempString += tens[number];
          } else { // otherwise 
            let [twoDigit, oneDigit] = number.split("");
            tempString += tens[twoDigit.slice(0,1) + "0"] + "-" + single[oneDigit];
          }
        } else if (number.length === 1) { // If it is only 1 digit add
